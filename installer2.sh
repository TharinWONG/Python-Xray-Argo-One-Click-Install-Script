...
if [ "$1" = "-v" ]; then
    if [ -f "$NODE_INFO_FILE" ]; then
        echo -e "${GREEN}========================================${NC}"
        echo -e "${GREEN}           節點信息查看               ${NC}"
        echo -e "${GREEN}========================================${NC}"
        echo
        cat "$NODE_INFO_FILE"
        echo
    else
        echo -e "${RED}未找到節點信息文件${NC}"
        echo -e "${YELLOW}請先運行部署腳本生成節點信息${NC}"
    fi
    exit 0
fi
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}    Python Xray Argo 一鍵部署腳本    ${NC}"
echo -e "${GREEN}========================================${NC}"
echo
...
echo
echo -e "${GREEN}本腳本基于 eooce 大佬的 Python Xray Argo 項目開發${NC}"
...
echo -e "${BLUE}3) 查看節點信息 - 顯示已保存的節點信息${NC}"
echo
read -p "請輸入選擇 (1/2/3): " MODE_CHOICE
...
if [ "$MODE_CHOICE" = "3" ]; then
    if [ -f "$NODE_INFO_FILE" ]; then
        ...
        cat "$NODE_INFO_FILE"
        echo
        echo -e "${YELLOW}提示: 如需重新部署，請重新運行腳本選擇模式1或2${NC}"
    else
        echo
        echo -e "${RED}未找到節點信息文件${NC}"
        echo -e "${YELLOW}請先運行部署腳本生成節點信息${NC}"
        echo
        echo -e "${BLUE}是否現在開始部署? (y/n)${NC}"
        read -p "> " START_DEPLOY
        if [ "$START_DEPLOY" = "y" ] || [ "$START_DEPLOY" = "Y" ]; then
            echo -e "${YELLOW}請選擇部署模式:${NC}"
            echo -e "${BLUE}1) 極速模式${NC}"
...
if [ "$MODE_CHOICE" = "1" ]; then
    ...
    echo -e "${GREEN}YouTube分流已自動配置${NC}"
...
# 修改Python文件添加YouTube分流到xray配置，並增加80端口節點
echo -e "${BLUE}正在添加YouTube分流功能和80端口節點...${NC}"
cat > youtube_patch.py << 'EOF'
# 讀取app.py文件
with open('app.py', 'r', encoding='utf-8') as f:
    content = f.read()

# 找到原始配置並替換爲包含YouTube分流的配置
...
print("YouTube分流配置和80端口節點已成功添加")
EOF

python3 youtube_patch.py
rm youtube_patch.py

echo -e "${GREEN}YouTube分流和80端口節點已集成${NC}"
...
if [ -z "$APP_PID" ] || [ "$APP_PID" -eq 0 ]; then
    echo -e "${RED}獲取進程PID失敗，嘗試直接啓動${NC}"
    nohup python3 app.py > app.log 2>&1 &
    sleep 2
    APP_PID=$(pgrep -f "python3 app.py" | head -1)
    if [ -z "$APP_PID" ]; then
        echo -e "${RED}服務啓動失敗，請檢查Python環境${NC}"
...
if ! ps -p "$APP_PID" > /dev/null 2>&1; then
    ...
    echo -e "${YELLOW}查看日志: tail -f app.log${NC}"
    echo -e "${YELLOW}檢查端口占用: netstat -tlnp | grep :3000${NC}"
    exit 1
fi
...
echo -e "${YELLOW}正在等待Argo隧道建立和節點生成，請耐心等待...${NC}"
...
while [ $WAIT_COUNT -lt $MAX_WAIT ]; do
    ...
    if [ $((WAIT_COUNT % 30)) -eq 0 ]; then
        ...
        echo -e "${YELLOW}已等待 ${MINUTES}分${SECONDS}秒，繼續等待節點生成...${NC}"
        echo -e "${BLUE}提示: Argo隧道建立需要時間，請繼續等待${NC}"
    fi
...
# 檢查是否成功獲取到節點信息
if [ -z "$NODE_INFO" ]; then
    echo -e "${RED}等待超時！節點信息未能在10分鍾內生成${NC}"
    ...
    echo -e "2. 檢查服務: ${YELLOW}ps aux | grep python3${NC}"
    echo -e "3. 重新運行腳本"
    echo
...
echo -e "${GREEN}節點配置:${NC}"
echo "$DECODED_NODES"
echo

echo -e "${GREEN}訂閱鏈接:${NC}"
echo "$NODE_INFO"
echo
...
if command -v curl &> /dev/null; then
    ...
    if [ "$PUBLIC_IP" != "獲取失敗" ]; then
        SAVE_INFO="${SAVE_INFO}
訂閱地址: http://$PUBLIC_IP:$SERVICE_PORT/$SUB_PATH_VALUE
管理面板: http://$PUBLIC_IP:$SERVICE_PORT"
    fi
fi

SAVE_INFO="${SAVE_INFO}
本地訂閱: http://localhost:$SERVICE_PORT/$SUB_PATH_VALUE
...
停止服務: kill $APP_PID
重啓服務: kill $APP_PID && nohup python3 app.py > app.log 2>&1 &
查看進程: ps aux | grep python3

=== 分流說明 ===
- 已集成YouTube分流優化到xray配置
- YouTube相關域名自動走專用線路
- 無需額外配置，透明分流"

echo "$SAVE_INFO" > "$NODE_INFO_FILE"
echo -e "${GREEN}節點信息已保存到 $NODE_INFO_FILE${NC}"
echo -e "${YELLOW}使用腳本選擇選項3可隨時查看節點信息${NC}"

echo -e "${YELLOW}=== 重要提示 ===${NC}"
echo -e "${GREEN}部署已完成，節點信息已成功生成${NC}"
echo -e "${GREEN}可以立即使用訂閱地址添加到客戶端${NC}"
echo -e "${GREEN}YouTube分流已集成到xray配置，無需額外設置${NC}"
echo -e "${GREEN}服務將持續在後台運行${NC}"
echo
